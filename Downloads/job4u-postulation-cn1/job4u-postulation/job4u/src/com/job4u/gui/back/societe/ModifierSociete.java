...
public class ModifierSociete extends Form {
    ...
    public ModifierSociete(Form previous) {
        super("Modifier", new BoxLayout(BoxLayout.Y_AXIS));
        this.previous = previous;

        currentSociete = AfficherToutSociete.currentSociete;
    ...
    private void addGUIs() {
        ...
        adresseTF.setText(currentSociete.getAdresse());
        emailTF.setText(currentSociete.getEmail());
        telTF.setText(currentSociete.getTel());
        domaineTF.setText(currentSociete.getDomaine());


        if (currentSociete.getImage() != null) {
            selectedImage = currentSociete.getImage();
            String url = Statics.SOCIETE_IMAGE_URL + currentSociete.getImage();
    ...
    private void addActions() {
        ...
        manageButton.addActionListener(action -> {
            if (controleDeSaisie()) {
                int responseCode = SocieteService.getInstance().edit(
                        new Societe(
                                currentSociete.getId(),
                ...
                if (responseCode == 200) {
                    AlertUtils.makeNotification("Societe modifié avec succes");
                    showBackAndRefresh();
                } else {
                    Dialog.show("Erreur", "Erreur de modification de societe. Code d'erreur : " + responseCode, new Command("Ok"));
                }
            }
        });
    }
