...
public class ModifierPostulation extends Form {
    ...
    public ModifierPostulation(Form previous) {
        super("Modifier", new BoxLayout(BoxLayout.Y_AXIS));
        this.previous = previous;

        currentPostulation = AfficherToutPostulation.currentPostulation;
    ...
    private void addGUIs() {
        ...
        adresseTF.setText(currentPostulation.getAdresse());
        emailTF.setText(currentPostulation.getEmail());
        dateTF.getPicker().setDate(currentPostulation.getDate());

        userPC.getPicker().setSelectedString(currentPostulation.getUser().getEmail());
        selectedUser = currentPostulation.getUser();
    ...
    private void addActions() {
        ...
        manageButton.addActionListener(action -> {
            if (controleDeSaisie()) {
                int responseCode = PostulationService.getInstance().edit(
                        new Postulation(
                                currentPostulation.getId(),
                ...
                if (responseCode == 200) {
                    AlertUtils.makeNotification("Postulation modifié avec succes");
                    showBackAndRefresh();
                } else {
                    Dialog.show("Erreur", "Erreur de modification de postulation. Code d'erreur : " + responseCode, new Command("Ok"));
                }
            }
        });
    }
